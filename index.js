...
if (missingOptionalEnv.length > 0) {
  console.warn(`Missing optional environment variables: ${missingOptionalEnv.join(', ')}. Related features will be disabled until configured.`);
}
...
const config = {
  ...
  alertRoleIds: ['532448115861749770'],
  reviewerIds: ['532448115861749770']
};
...
function ensureConfig(key, message) {
...
async function sendApplicationToReviewChannel(guild, applicant, details, application) {
  ensureConfig('applicationChannelId', 'APPLICATION_CHANNEL_ID is not configured.');
...
async function createSupportTicket(interaction, user) {
  ensureConfig('supportCategoryId', 'SUPPORT_CATEGORY_ID is not configured.');
...
async function handleApproveButton(interaction, applicationId) {
  ensureConfig('creatorPendingRoleId', 'CREATOR_PENDING_ROLE_ID is not configured.');
...
async function handleActivateCommand(interaction) {
  ensureConfig('creatorPendingRoleId', 'CREATOR_PENDING_ROLE_ID is not configured.');
  ensureConfig('creatorRoleId', 'CREATOR_ROLE_ID is not configured.');
...
async function handleTicketClose(interaction, ownerId) {
  ...
  const isSupportReviewer = interaction.member.roles.cache.some((role) => config.alertRoleIds.includes(role.id));
  const canManageChannels = interaction.member.permissions.has(PermissionsBitField.Flags.ManageChannels);

  if (!isTicketOwner && !isSupportReviewer && !canManageChannels) {
    await interaction.editReply({ content: 'Only ticket owner or support staff can close this ticket.' });
    return;
  }
